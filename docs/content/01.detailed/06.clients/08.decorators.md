...
| id_generator | Callable | キャッシュの識別子を生成する関数。                                 |
...
2. 引数 `f` が `None` の場合、つまり引数ありで使用された場合は、引数 `id_generator` と `timeout` を反映した、初期化関数を返す。
3. 初期化関数は、以下のように引数 `f` に受けとった関数をラッパーする。
   ...
   3. 関数の完全インポート名と、引数をもとに `"<key_prefix><import_name>-<identifier>-<args_hash>"` の形式でキャッシュのキーを生成する。
      ...
      - `identifier`： 引数 `f` の関数に渡された第一引数の値。あるいは引数 `id_generator` によって生成された識別子。
